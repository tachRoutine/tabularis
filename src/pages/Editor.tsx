...
export const Editor = () => {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      setError("Failed to export CSV: " + String(e));
    }
  };
  ...
  const handleExportJSON = async () => {
    ...
    try {
      ...
      console.error("Export failed:", e);
      setError("Failed to export JSON: " + String(e));
    }
  };
                    ...
                    selectableQueries.map((q, i) => (
                      ...
                      >
                        <button
                          ...
                          title={q}
                        >
                          <div className="mt-0.5 min-w-[10px] shrink-0">
                            <Play
                              size={10}
          ...
          <div className="flex-1 min-h-0 flex flex-col">
            <div className="p-2 bg-slate-900 text-xs text-slate-400 border-b border-slate-800 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-4">
                <span>
                  {result.rows.length} rows retrieved
                  {executionTime !== null && (
              ...
              {result.pagination && result.pagination.total_rows > 0 && (
                <div className="flex items-center gap-2">
                  <span className="mr-2">
                    ...
                    -{" "}
                    {Math.min(
                      result.pagination.page * result.pagination.page_size,
                      result.pagination.total_rows,
                    )}{" "}
                    of {result.pagination.total_rows}
                  </span>
                  ...
                  <div className="flex items-center bg-slate-800 rounded border border-slate-700">
                    ...
                    <button
                      disabled={result.pagination.page <= 1 || isLoading}
                      onClick={() =>
                        runQuery(query, result.pagination!.page - 1)
                      }
                    ...
                    <button
                      disabled={
                        result.pagination.page * result.pagination.page_size >=
                          result.pagination.total_rows || isLoading
                      }
                      onClick={() =>
                        runQuery(query, result.pagination!.page + 1)
                      }
